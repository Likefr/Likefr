<p align="center"><img src="github-header-image (1).png"></p>

<h1 align="center">嗨 👋，我是 Likefr</h1>

<p align="center">
  <em>迷失的人迷失了，相逢的人会再相逢</em>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=likefr&label=Profile%20views&color=0e75b6&style=flat" alt="likefr" />
</p>

---

### 🧑‍💻 关于我

- 🔧 **全栈工程师** — Java × Vue × PostgreSQL × Docker × AI
- 🧠 **SummerMemory** — 自建 AI 记忆系统，纯本地向量搜索 + 图谱可视化 → [在线 Demo](https://ai.likefr.com/graph)
- 📱 **SQL Studio** — Android 数据库管理工具
- 📝 我经常在 [likefr.com](https://likefr.com) 上写技术文章
- 📫 联系我：**likefr@foxmail.com**

---

### 🛠️ 技术栈

**后端：** Java / Spring Boot / MyBatis-Plus / Netty / Kafka / PostgreSQL / Redis / Elasticsearch

**前端：** Vue / Element-UI / 微前端 (microApp)

**运维：** Docker / Nginx / Linux / FRP / 群晖 NAS

**AI & IoT：** Ollama / pgvector / ESP32 / 小米 IoT

**移动端：** Android (Java)

---

### 🚀 开源项目

| 项目 | 描述 | 链接 |
|------|------|------|
| 🧠 SummerMemory | 自建 AI 记忆系统 · 向量搜索 + 图谱可视化 | [Demo](https://ai.likefr.com/graph) |
| 📱 SQL Studio | Android 数据库管理工具 | 开发中 |
| 📝 likefr.com | 技术博客 · 实战踩坑记录 | [访问](https://likefr.com) |

---

### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=likefr&show_icons=true&theme=tokyonight&hide_border=true" alt="likefr" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs?username=likefr&show_icons=true&locale=en&layout=compact&theme=tokyonight&hide_border=true" alt="likefr" />
</p>

---

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" alt="java" width="35" height="35" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/vuejs/vuejs-original-wordmark.svg" alt="vuejs" width="35" height="35" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postgresql/postgresql-original.svg" alt="postgresql" width="35" height="35" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg" alt="docker" width="35" height="35" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/linux/linux-original.svg" alt="linux" width="35" height="35" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/android/android-original-wordmark.svg" alt="android" width="35" height="35" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" alt="git" width="35" height="35" />
</p>
